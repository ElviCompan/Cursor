    ...
    def apply_limits_and_place(self, disk_bytes: int) -> tuple[list[list[Segment]], list[int], int, int]:
        ...
        for row in self.luns:
            ...
            if spec.usable > 0 and chosen is None:
                ...
                warn = "потолок по свободным кускам"
            row.set_warning(warn)
...
def _html_disk_bar(segments: list[Segment], free: int, disk_bytes: int) -> str:
    ...
    for seg in segments:
        ...
        parts.append(
            f'<div class="seg{kind_cls}" style="width:{pct:.4f}%;background:{seg.color}" '
    ...
    for tab in app.tabs:
        ...
        for row in tab.luns:
            ...
            lun_rows.append(
                ...
                f"<td>{spec.width}</td>"
                f"<td>{format_tb(spec.usable)} ТБ</td>"
                f"<td>{format_tb(ext)} ТБ</td>"
        ...
        for i in range(n):
            ...
            disk_rows.append(
                ...
                f"<div class='dused'>{format_tb(used)} / {format_tb(disk_bytes)} ТБ</div>"
                "</div>"
            )
        sections.append(
            f"<section><h2>{html.escape(tab.title)}</h2>"
            f"<p>Дисков: {n}. Занято сырого: {format_tb(used_raw)} ТБ. "
            f"Полезно: {format_tb(tab._last_usable)} ТБ. Чётность: {format_tb(tab._last_parity)} ТБ.</p>"
            "<table><thead><tr><th>LUN</th><th>RAID</th><th>Дисков</th>"
            "<th>Полезно</th><th>Кусок</th><th>Данные / чётность</th></tr></thead><tbody>"
...
<style>
  ...
  .dused {{ width: 140px; font-size: 12px; color: #334155; }}
...
<p>Пул: {count} × {tb:g} ТБ = {format_tb(pool_bytes)} ТБ ({format_bytes(pool_bytes)} Б).
В группах: {used_disks} диск(ов), свободно: {remaining_disks}.
Суммарно полезно: {format_tb(total_usable)} ТБ, чётность: {format_tb(total_parity)} ТБ.</p>
{body}
...
class RaidPlannerApp(ttk.Frame):
    def __init__(self, master: tk.Tk) -> None:
        ...
        self.disk_tb_var = tk.StringVar(value="2.0")
    ...
    def _build_groups_section(self) -> None:
        box = ttk.LabelFrame(self, text="Группы DDP", padding=8)
